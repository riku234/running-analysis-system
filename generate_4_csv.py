#!/usr/bin/env python3
"""
全フレーム骨格データCSV生成スクリプト - 「4」
Video ID: 42976759-5dd9-4307-bedf-6424ab0ce9f9 の全フレーム骨格データをCSV形式で出力します
"""
...
# 動画ID
VIDEO_ID = "42976759-5dd9-4307-bedf-6424ab0ce9f9"
...
def save_pose_data_to_csv(pose_data, video_info, filename="4.csv"):
    """pose_dataをCSV形式で保存"""
    if not pose_data:
...
def main():
    ...
    if pose_data and video_info:
        # CSVファイルを生成
        success = save_pose_data_to_csv(pose_data, video_info, filename="4.csv")
        
        if success:
            print("\n🎉 処理完了! CSVファイル「4.csv」が生成されました")
        else:
            print("\n❌ 処理失敗")
    else:
