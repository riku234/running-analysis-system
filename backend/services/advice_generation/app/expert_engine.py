...
1. 専門家の見解をベースに、励ますような自然な日本語で記述すること。
2. JSON形式のみで出力すること。
3. マークダウン記法（#, *, -, **など）は一切使用しないこと。
...
"""
    ...
    try:
        ...
        advice_data = {
            "title": "フォーム改善アドバイス",
            "message": f"専門家の分析により、{len(diagnosed_issues)}つの改善点が見つかりました。{'、'.join(issue_names)}の傾向が見られます。専門家の見解に基づき、段階的にフォーム改善に取り組んでください。",
            "key_points": issue_names[:3]  # 最大3つまで
        }
        ...
        advice_data = {
            "title": "フォーム改善アドバイス",
            "message": f"改善点が{len(diagnosed_issues)}つ見つかりました。専門家の見解に基づき、段階的にフォーム改善に取り組んでください。",
            "key_points": issue_names[:3]
        }
        print(f"🔄 フォールバックアドバイスを使用: {advice_data.get('title')}")
