...
1. 専門家の見解（現象・原因・改善策・ドリル）を読みやすい形式で整理して記述すること。
2. 不要な励ましの文章や装飾的な表現は一切使用しないこと。
3. 簡潔で実践的なアドバイスにすること。
4. JSON形式のみで出力すること。
5. マークダウン記法（#, *, -, **など）は一切使用しないこと。

【出力形式の例】

検出された課題の現象・原因・改善策・ドリルを読みやすく整理した形式で出力してください。
例：
「【現象】: 重心が後ろに残り、ブレーキがかかっています。脚の筋肉への負担が大きい走り方です。
【原因】: 骨盤が後傾しており、着地時にお尻が落ちてしまっています。
【改善策】: おへそからロープで前に引っ張られるようなイメージを持ち、自然な前傾姿勢を作りましょう。
【ドリル】: ウォールドリル（壁押し）」
...
{{
    "title": "タイトル（簡潔に）",
    "message": "メインメッセージ（専門家の見解を読みやすく整理した形式、200-300文字程度）",
