#!/usr/bin/env python3
"""
全フレーム骨格データCSV生成スクリプト - 「前傾」
Video ID: ac586bb2-914f-4e63-9707-3cc2dd92b445 の全フレーム骨格データをCSV形式で出力します
"""
...
# 動画ID
VIDEO_ID = "ac586bb2-914f-4e63-9707-3cc2dd92b445"
...
def save_pose_data_to_csv(pose_data, video_info, filename="前傾.csv"):
    """pose_dataをCSV形式で保存"""
    if not pose_data:
...
def main():
    ...
    if pose_data and video_info:
        # CSVファイルを生成
        success = save_pose_data_to_csv(pose_data, video_info, filename="前傾.csv")
        
        if success:
            print("\n🎉 処理完了! CSVファイル「前傾.csv」が生成されました")
        else:
            print("\n❌ 処理失敗")
    else:
